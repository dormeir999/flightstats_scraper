...
from utils_flightstat import create_list_of_airports, get_flights_links
...
def get_flight_details(soup):
    ...
    flight_details_dict = {'flight_number': soup.find(CFG.FLIGHT_CARRIER_STRING,
                                                      class_=CFG.FLIGHT_CARRIER_CLASS).string.split(
        CFG.FLIGHT_CARRIER_SPACE_STR)
    [-CFG.FOURTH_ITEM],
                           ...
                           'departure_date': soup.find_all(CFG.FLIGHT_STAT_STR, class_=CFG.DATE_CLASS)
                           [CFG.FIRST_ITEM].string,
                           'arrival_date': soup.find_all(CFG.FLIGHT_STAT_STR, class_=CFG.DATE_CLASS)
                           [CFG.THIRD_ITEM].string,
...
def get_flight_events(soup):
    ...
    for i, b in enumerate(a):
        if not b.string:  # empty cell
            ...
            time_iter += 1
            if time_iter % CFG.DIVISOR_UTC == 0:  # is UTC time
            ...
            string_before = b.string
            flight_events.append(b.string)
        elif len(b.string.split(" ")[CFG.SECOND_ITEM]) == CFG.MONTH_STRING_LENGTH:  # is date, len 3 is month short name
            string_before = b.string
            flight_events.append(b.string)
...
def get_flights_data(flight_links):
    ...
    if isinstance(flight_links, tuple):  # if more than one page, iterate over the links part (not the bool)
        for flight_link in flight_links[CFG.FIRST_ITEM]:
    ...
    else:  # It's just a list, so iterate over everything
        for flight_link in flight_links:
            ...
            flight_details.append(
                flight_detail_temp)  # Scrape for regular flight's details (name, destination, gate)
...
def main():
    ...
    parser.add_argument("-type", type=str, nargs='+', choices=CFG.AIRPORTS_TYPES)
